...
compareDivSeqList = new ArrayList();
...
if(UtilValidate.isNotEmpty(preRetrieved) && preRetrieved != "N") {
    ...
    for (toMapEntry in divSeqToList) {
        if (initial || searchKeysNotInYourLabelFile.equals("Y")) {
            compareDivSeqList.add(UtilMisc.toMap("key", toMapEntry.key,"screen",toMapEntry.screen, "fromMap", new HashMap(), "toMap", toMapEntry, "type", "2"));
        }
    }
}
