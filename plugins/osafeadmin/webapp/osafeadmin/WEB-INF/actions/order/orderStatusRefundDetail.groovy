...
{
	orderHeader = delegator.findOne("OrderHeader", [orderId : parameters.orderId]);
}
...
List<String> processedReturnIds = new ArrayList();
...
List orderItemSequenceIds = new ArrayList();
Map returnItemSeqIdQuantityMap = new HashMap();
List<GenericValue> returnItems = new ArrayList();
...
for(int i = 0; i < orderItems.size() ; i++)
{
	...
	if(UtilValidate.isNotEmpty(orderItemSeqId))
	{
		...
		orderItem = delegator.findOne("OrderItem", [orderId : parameters.orderId, orderItemSeqId: orderItemSeqId]);
...
if(statusId.equalsIgnoreCase("ORDER_CANCELLED"))
{
	Map svcShippingCtx = new HashMap();
