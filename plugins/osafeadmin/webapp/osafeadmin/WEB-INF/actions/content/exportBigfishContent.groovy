...
passedContentTypeIds = new ArrayList();
passedProdCatContentTypeIds = new ArrayList();
...
partyContactMechPurposeTypeExpr = new ArrayList();
...
if (UtilValidate.isNotEmpty(exportFile)) 
{
    ...
    for(contentTypeId in passedContentTypeIds) 
    {
        ...
        try {
            ...
            contentType = delegator.findOne("ContentType", ["contentTypeId" : contentTypeId]);
            if (contentType!= null) 
            {
    ...
    for(prodCatContentTypeId in passedProdCatContentTypeIds) 
    {
        ...
        try {
            TransactionUtil.commit(beganTransaction);
            contentType = delegator.findOne("ProductCategoryContentType", ["prodCatContentTypeId" : prodCatContentTypeId]);
            if (contentType!= null) 
            {
                //contentType.writeXmlText(writer, "");
                numberWritten++;
