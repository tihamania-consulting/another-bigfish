...
public class OsafeAdminEvent {
    ...
    public static String catalogAssetChecker(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        try
        {
            ...
            List<String> ProductContentTypeList = new ArrayList();
            ...
            for (Map<String, Object> workingCategoryMap : productCategories) 
            {
               ...
               if(UtilValidate.isNotEmpty(workingCategory))
               {
                  String categoryImageURL =workingCategory.getString("categoryImageUrl");
                   ...
                   List<GenericValue> productCategoryMembers = EntityUtil.filterByDate(delegator.findByAnd("ProductCategoryMember", UtilMisc.toMap("productCategoryId", workingCategory.getString("productCategoryId")), UtilMisc.toList(), true));
                   for (GenericValue productCategoryMember : productCategoryMembers) 
                   {
                       ...
                       if(UtilValidate.isNotEmpty(product))
                       {
                           ...
                           {
                               List<GenericValue> virtualProductAssocs = delegator.findByAnd("ProductAssoc", UtilMisc.toMap("productId", productId, "productAssocTypeId", "PRODUCT_VARIANT"), UtilMisc.toList("-fromDate"), true);
