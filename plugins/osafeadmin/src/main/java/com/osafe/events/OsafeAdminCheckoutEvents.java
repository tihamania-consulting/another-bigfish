...
public class OsafeAdminCheckoutEvents {
	...
	public static String addOrderAdjustment(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        if(UtilValidate.isNotEmpty(cart))
        {
	    	...
	    	{
	    		orderAdjustmentList = new ArrayList();
	    	}
	        ...
	        if(UtilValidate.isNotEmpty(orderAdjustmentId))
	        {
	            ...
	            Map<String, Object>  orderAdjustmentInfoMap = new HashMap();
    ...
    public static String processOrderAdjustmentAttributes(HttpServletRequest request, HttpServletResponse response) {
    	...
    	if (UtilValidate.isNotEmpty(orderId))
    	{
    		...
    		try
    		{
    			orderHeader = EntityQuery.use(delegator).from("OrderHeader").where(UtilMisc.toMap("orderId", orderId)).queryOne();
	    	} 
    		catch (Exception e) 
	    	{
	            Debug.logError(e, module);
    ...
    public static String processPayment(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        try 
    	{            
        	sysUserLogin = EntityQuery.use(delegator).from("UserLogin").where(UtilMisc.toMap("userLoginId", "system")).queryOne();
        } 
    	catch (Exception e) 
    	{
		    	...
		    	else
		    	{
		    		...
		    		try {
		    			...
		    			if ("ORDER_HOLD".equals(orderHeader.getString("statusId")))
		    			{
                            ...
                            Map<String, String> emailContext = UtilMisc.toMap("orderId", orderId, "userLogin", (String) sysUserLogin.get("userLogin"));
