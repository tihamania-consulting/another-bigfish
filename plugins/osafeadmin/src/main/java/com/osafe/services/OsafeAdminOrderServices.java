...
public class OsafeAdminOrderServices 
{
    ...
    public static Map recalcOrderShippingAmount(DispatchContext ctx, Map context) 
    {
        ...
        try 
        {
            orderHeader = delegator.findOne("OrderHeader", UtilMisc.toMap("orderId", orderId));
        } 
	            ...
	            while (itr.hasNext()) 
	            {
	                ...
	                if (UtilValidate.isNotEmpty(validOrderItems)) 
	                {
	                    ...
	                    for (int i = 0; i < validOrderItems.size(); i++) 
	                    {
	                        ...
	                        try 
	                        {
	                            products.add(i, delegator.findOne("Product", UtilMisc.toMap("productId", productId)));  // get the product entity
            ...
            for (GenericValue item : orderItems) 
            {
                ...
                else 
                {
                    ...
                    if (workEffortId != null) 
                    {
                        try 
                        {
                            workEffort = delegator.findOne("WorkEffort", UtilMisc.toMap("workEffortId", workEffortId));
                        } catch (GenericEntityException e) {
                            Debug.logError(e, module);
                        }
                    }
                ...
                try 
                {
                    cartItem.setItemGroup(cart.addItemGroup(item.getRelatedOne("OrderItemGroup")));
                } catch (GenericEntityException e) {
                    Debug.logError(e, module);
                    return ServiceUtil.returnError(e.getMessage());
                }
