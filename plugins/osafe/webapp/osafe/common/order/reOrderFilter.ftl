             ...
             <#if customerOrderList?has_content>
              <#list customerOrderList as customerOrder>
                <option value = "${customerOrder.orderId}" <#if parameters.orderId?exists && parameters.orderId == customerOrder.orderId>Selected</#if>>
                  ${Static["org.apache.ofbiz.base.util.UtilProperties"].getMessage("OSafeUiLabels", "OrderPlacedOnDateLabel", Static["org.apache.ofbiz.base.util.UtilMisc"].toMap("orderId", customerOrder.orderId, "orderDate", Static["com.osafe.util.Util"].convertDateTimeFormat(customerOrder.orderDate, preferredDateFormat)), locale)!}
                </option>
              </#list>
             </#if>
           </select>
