...
productCategoryMemberList = new ArrayList();
...
{
	List sortedVariantList = new ArrayList();
...
if (UtilValidate.isNotEmpty(productId)) 
{
 ...
 // Setting variables required in the Manufacturer Info section 
    if (UtilValidate.isNotEmpty(gvProduct)) 
    {
        ...
        {
            productCategoryMemberList = gvProduct.getRelated("ProductCategoryMember");
        ...
        if (UtilValidate.isNotEmpty(productCategoryId)) 
        {
            ...
            if (UtilValidate.isNotEmpty(gvProductCategory))
            {
                ...
                productCategoryContentList = gvProductCategory.getRelated("ProductCategoryContent");
                ...
                if (productCategoryContentList) 
                {
                  ...
                  if (UtilValidate.isNotEmpty(pdpAddCategoryContentList)) 
                  {
                     ...
                     context.pdpEspotContent = pdpAddCategoryContent.getRelatedOne("Content");
                  }
                }
            }
        }
    ...
    if (UtilValidate.isNotEmpty(gvProduct)) 
    {
        ...
        productContentList = gvProduct.getRelated("ProductContent");
        ...
        productAttr = gvProduct.getRelated("ProductAttribute");
        ...
        if(pdpRecentViewedMax > 0)
        {
            ...
            lastViewedProductsClone = new ArrayList();
        ...
        volumePricingRule = [];
        volumePricingRuleMap = new HashMap();
        ...
        if (UtilValidate.isNotEmpty(productIdConds))
        {
            for (GenericValue priceCond: productIdConds) 
            {
                ...
                {
                    qtyBreakIdConds = priceRule.getRelated("ProductPriceCond");
                    ...
                    {
                        priceIdActions = priceRule.getRelated("ProductPriceAction");
        ...
        context.productReviews = new ArrayList();
        ...
        if(UtilValidate.isNotEmpty(reviewMethod))
        {
            if(reviewMethod.equalsIgnoreCase("BIGFISH"))
            {
			    ...
			    productCalculatedInfos = gvProduct.getRelated("ProductCalculatedInfo");
    		    ...
    		    reviews = gvProduct.getRelated("ProductReview");
		...
		if(UtilValidate.isNotEmpty(productCategoryId))
		{
			...
			productFeatureCatGrpAppls = delegator.findByAnd("ProductFeatureCatGrpAppl", UtilMisc.toMap("productCategoryId", productCategoryId), UtilMisc.toList("sequenceNum"));
		    ...
		    distinguishingFeatures = delegator.findByAnd("ProductFeatureAndAppl", UtilMisc.toMap("productId", gvProduct.productId, "productFeatureApplTypeId", "DISTINGUISHING_FEAT"), UtilMisc.toList("sequenceNum"));
        ...
        productAssoc = gvProduct.getRelated("MainProductAssoc");
        ...
        complementProducts = new ArrayList();
        ...
        Map productInventoryMap = new HashMap();
        ...
        if ("Y".equals(gvProduct.isVirtual)) 
        {
            ...
            else 
            {
                ...
                selectableFeatures = delegator.findByAnd("ProductFeatureAndAppl", UtilMisc.toMap("productId", gvProduct.productId, "productFeatureApplTypeId", "SELECTABLE_FEATURE"), UtilMisc.toList("sequenceNum"));
                ...
                if (UtilValidate.isNotEmpty(featureSet)) 
                {
                    ...
                    Map productFeatureAndApplSelectMap = new HashMap();
                    Map productVariantDisFeatureMap = new HashMap();
                    Map productVariantStandardFeatureMap = new HashMap();
                    Map productVariantContentWrapperMap = new HashMap();
                    Map productVariantProductMap = new HashMap();
                    Map productVariantPriceMap = new HashMap();
                    Map variantVolumePricingRuleMap = new HashMap();
                    Map variantVolumePricingRuleMapMap = new HashMap();
                    Map productVariantInventoryMap = new HashMap();
                    Map productVariantProductContentIdMap = new HashMap();
                    Map productVariantContentMap = new HashMap();
					Map productVariantProductAttributeMap = new HashMap();
                    Map productFeatureFirstVariantIdMap = new HashMap();
					Map productFacetTooltipMap = new HashMap();
                    
                    //IF PRODUCT CATEGORY BUILD THE FEATURE SET BASED ON THE PRODUCT_FEATURE_CAT_GRP_APPL (SEQUENCE)
                    if(UtilValidate.isNotEmpty(gvProductCategory))
                    {
                        productFeatureCatGroupAppls = gvProductCategory.getRelated("ProductFeatureCatGrpAppl");
                            ...
                            if(ProductWorker.isSellable(assocVariantProduct))
                            {
                                ...
                                if (UtilValidate.isNotEmpty(productVariantContentList))
                                {
                                    ...
                                    for (GenericValue productContent: productVariantContentList) 
                                    {
                                       ...
                                       variantContent = productContent.getRelatedOne("Content");
                                ...
                                volumePricingRule = [];
                                volumePricingRuleMap = new HashMap();
                                productIdConds = delegator.findByAnd("ProductPriceCond", [inputParamEnumId: "PRIP_PRODUCT_ID", condValue: assocVariantProduct.productId],["productPriceRuleId"]);
                                if (UtilValidate.isNotEmpty(productIdConds))
                                {
                                    for (GenericValue priceCond: productIdConds) 
                                    {
								...
								productVariantFeatureTypes = new ArrayList();
								productVariantFeaturesByType = new LinkedHashMap();
								Map variantDistinguishingFeatureMap = new HashMap();
										...
										if (UtilValidate.isNotEmpty(distFeaturesInGroup))
										{
											for (GenericValue distfeature: distFeaturesInGroup)
											{
												...
												if (!productVariantFeatureTypes.contains(featureType))
												{
													...
													if (UtilValidate.isNotEmpty(productFeatureGroupAppls))
													{
														prodVariantFeaturesList = new ArrayList();
														for(GenericValue productFeatureGroupAppl: productFeatureGroupAppls)
														{
															//pull the feature from the list of distinguishing features we already have
															features = EntityUtil.filterByAnd(distFeaturesInGroup, UtilMisc.toMap("productFeatureId", productFeatureGroupAppl.productFeatureId));
