...
set SOLR_PARMS=-Dsolr.solr.home=%OFBIZ_HOME%/plugins/solr -Dsolr.data.dir=%OFBIZ_HOME%/plugins/solr/data

javaservice -install "@service.name@" %JAVA_HOME%\jre\bin\server\jvm.dll -Xms@Xms@ -Xmx@Xmx@ %SOLR_PARMS% -Djava.class.path=%JAVA_HOME%\lib\tools.jar;%OFBIZ_HOME%\ofbiz.jar -start org.apache.ofbiz.base.start.Start -out %LOGS_DIR%\serviceLog.txt -err %LOGS_DIR%\serviceErr.txt -current %OFBIZ_HOME% -manual
