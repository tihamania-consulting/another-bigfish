...
public class SolrEvents 
{
    ...
    public static String solrSearch(HttpServletRequest request, HttpServletResponse response) 
    {
        try 
        {
            ...
            Map<String, String> facetGroupDescriptions = new HashMap();
            Map<String, String> facetGroupIds = new HashMap();
            Map<String, String> facetGroupFacetSorts = new HashMap();

            //Build product Store Parms
            List<GenericValue> productStoreParms = delegator.findByAnd("XProductStoreParm",UtilMisc.toMap("productStoreId",productStoreId), null, true);
            ...
            Map<String, List> filterGroupParamMap = new HashMap();
            ...
            try 
            {
                ...
                if (UtilValidate.isNotEmpty(productCategoryId)) 
                {
                    productCategoryRollups = EntityUtil.filterByDate(delegator.findByAnd("ProductCategoryRollup", UtilMisc.toMap("productCategoryId", productCategoryId), UtilMisc.toList("sequenceNum"), true));
                ...
                List<String> facetCatGroups = new ArrayList();
                Map<String, String> facetCatGroupDescriptions = new HashMap();
                ...
                for (String sortOption: SORT_OPTIONS_LIST) 
                {
                    ...
                    if(OSAFE_PROPS.containsKey(sortOption))
                    {
                        ...
                        if (UtilValidate.isNotEmpty(sortOptionTxt))
                        {
                            ...
                            {
                                Map<String,String> sortOptionMap = new HashMap();
            ...
            if (UtilValidate.isNotEmpty(sortResults)) 
            {
                ...
                if (sortResultsParts.length > 1) 
                {
                	...
                	{
                        solrQueryFacet.addSort("salesDiscontinuationDateNullFlag", ORDER.desc);
                	}
                    ...
                    solrQueryFacet.addSort(sortName, solrOrder);
                }
            }
            else
            {
                solrQueryFacet.addSort("customerRating", ORDER.desc);
            }
            ...
            List<SolrIndexDocument> resultsCompleteMultiSelect = new ArrayList();
            if(facetMultiSelectTrue)
            {
    ...
    public static String autoSuggestionList(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        List<String> autoSuggestionList = new ArrayList();
        List<Long> autoSuggestionFreqList = new ArrayList();
    ...
    public static List<String> getAvailableFacilities(HttpServletRequest request)
    {
        ...
        List<String> availableFacilities = new ArrayList();
        ...
        try 
        {
            ...
            facilities = delegator.findByAnd("Facility", UtilMisc.toMap("productStoreId", productStoreId), null, true);
        } 
    ...
    public static GenericValue getShippingOriginContactMech(Delegator delegator, String partyId) throws GeneralException 
    {
        ...
        for (GenericValue address : addresses) 
        {
            ...
            {
                generalAddress = delegator.findOne("PostalAddress", UtilMisc.toMap("contactMechId", address.getString("contactMechId")), true);
            }
            else if ("SHIP_ORIG_LOCATION".equals(address.get("contactMechPurposeTypeId")))
            {
                originAddress = delegator.findOne("PostalAddress", UtilMisc.toMap("contactMechId", address.getString("contactMechId")), true);
    ...
    public static Set<String> expandGeoRegionDeep(Set<String> geoIdSet, Delegator delegator) throws GenericEntityException 
    {
        ...
        {
            List<GenericValue> geoAssocList = delegator.findByAnd("GeoAssoc", UtilMisc.toMap("geoIdTo", curGeoId, "geoAssocTypeId", "REGIONS"), null, true);
    ...
    public static String searchShoppingList(HttpServletRequest request, HttpServletResponse response) 
    {
        try 
        {
            ...
            List resultShoppingListSearch = new ArrayList();
            //Build product Store Parms
            List<GenericValue> productStoreParms = delegator.findByAnd("XProductStoreParm",UtilMisc.toMap("productStoreId",productStoreId), null, true);
            if (UtilValidate.isNotEmpty(productStoreParms))
            {
            ...
            List<String> processedSearchItems = new ArrayList();
            ...
            List resultShoppingListSearchPage = new ArrayList();
    ...
    public static QueryResponse getQueryResponse(String searchText, SolrClient solr, String nowDate)
    {
    	String queryFacet = null;
