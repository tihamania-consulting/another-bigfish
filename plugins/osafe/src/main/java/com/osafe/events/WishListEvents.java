...
public class WishListEvents {
    ...
    public static String getWishListId(HttpServletRequest request, boolean createNew) {
        ...
        try {
	        List<GenericValue> existingLists = delegator.findByAnd("ShoppingList", findMap, null, true);
            ...
            if (shoppingList != null) {
                ...
                while (sli.hasNext()) {
                    ...
                    if (productId != null) {
                        ...
                        GenericValue product = shoppingListItem.getRelatedOne("Product");
        ...
        while (parameterNameIter.hasNext())
        {
            ...
            if (parameterName.toUpperCase().startsWith("ADD_ITEM_ID"))
            {
                try
                {
                    ...
                    GenericValue shoppingListItem = delegator.findOne("ShoppingListItem", findMap, true);
                    ...
                    {
                        product = delegator.findOne("Product", UtilMisc.toMap("productId", productId), false);
                    	productName = ProductContentWrapper.getProductContentAsText(product, "PRODUCT_NAME", locale, dispatcher, "html");
                    	if(UtilValidate.isEmpty(productName))
                    	{
                    		GenericValue virtualProduct = ProductWorker.getParentProduct(productId, delegator);
                    		if(UtilValidate.isNotEmpty(virtualProduct))
