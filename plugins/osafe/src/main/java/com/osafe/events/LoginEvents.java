    ...
    public static String emailPassword(HttpServletRequest request, HttpServletResponse response) {
        ...
        try 
        {
            ...
            if (useEncryption) {
                ...
                supposedUserLogin.set("currentPassword", HashCrypt.digestHash(passwordToSend, LoginServices.getHashType().getBytes()));
        ...
        if (useEncryption) {
            ...
            request.setAttribute("_EVENT_MESSAGE_", errMsg);
            List<Object> osafeSuccessMessageList = UtilMisc.toList( errMsg);
            request.setAttribute("osafeSuccessMessageList", osafeSuccessMessageList);
        }
        		...
        		try 
        		{
                    ...
                    else
                    {
                    	...
                    	if (useEncryption) 
                    	{
                        	...
                        	userLoginFromFbEmail.set("currentPassword", HashCrypt.digestHash(passwordToUseForLogin, LoginServices.getHashType().getBytes()));
