...
public class CheckOutEvents {
    ...
    public static String autoCaptureAuthPayments(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
             ...
             */
            GenericValue sysLogin = delegator.findOne("UserLogin", UtilMisc.toMap("userLoginId", "admin"), false);
            
            List<GenericValue> lOrderPaymentPreference = delegator.findByAnd("OrderPaymentPreference", UtilMisc.toMap("orderId", orderId, "statusId", "PAYMENT_AUTHORIZED"), null, true);
            	...
            	for (Iterator<?> item = sc.iterator(); item.hasNext();) 
            	{
                	...
                	if (UtilValidate.isNotEmpty(sci.getShoppingListId()))
                	{
                        ...
                        try 
                        {
                            ...
                            GenericValue shoppingListItem = delegator.findOne("ShoppingListItem", UtilMisc.toMap("shoppingListId", shoppingListId,"shoppingListItemSeqId", (String) shoppingListItemResp.get("shoppingListItemSeqId")), false);
        ...
        if(UtilValidate.isNotEmpty(cart))
        {
	    	try {
	            ...
	            if(chosenShippingMethod.equalsIgnoreCase("NO_SHIPPING@_NA_"))
	            {
					...
					if(UtilValidate.isNotEmpty(taxedStoreId))
					{
						...
						if (UtilValidate.isNotEmpty(taxedParty))
						{
							...
							taxedPartyGeneralLocations = EntityUtil.getRelated("PartyContactMech", null, taxedPartyGeneralLocations, true);
	            ...
	            for (int shipGroupIdx = 0; shipGroupIdx < shipGroups; shipGroupIdx++) 
	            {
	                ...
	                List<EntityExpr> promoLoyaltyAdjExps = new ArrayList<>();
	                ...
	                Map<String, Object> serviceContext = UtilMisc.toMap("productStoreId", cart.getProductStoreId());
	                ...
	                serviceContext.put("itemProductList", new ArrayList());
	                serviceContext.put("itemAmountList", new ArrayList());
	                serviceContext.put("itemPriceList", new ArrayList());
	                serviceContext.put("itemShippingList", new ArrayList());
    ...
    public static String setShipGroups(HttpServletRequest request, HttpServletResponse response) 
    {
        ...
        Map<String, Object> shippingContactMechMap = new HashMap<>();
        Map<String, Object>  cartLineShippingContactMechQtyMap = new HashMap<>();
        Map<String, Object>  cartLineShippingContactMechGiftMsgMap = new HashMap<>();
        Map<String, Object>  cartLineQtyMap = new HashMap<>();
        Map<String, Object>  cartLineProductInfoMap  = new HashMap<>();
        Map<String, Object>  cartItemQtyMap = new HashMap<>();
        ...
        {
    		Map<ShoppingCartItem, String> cartItemMessageCount = new HashMap();
        ...
        if(isCheckoutSplitShipGroups)
        {
	    	try 
	    	{
	            ...
	            for (int i = 0; i < lCartShipGroups.size(); i++) 
	            {
	                ...
	                Set<ShoppingCartItem> shipItems = shipGroupShipInfo.getShipItems();
	                if (UtilValidate.isNotEmpty(shipItems) && shipItems.size() > 1)
	                {
	            		StringBuffer sbProductStoreShipMethIdKey= new StringBuffer();
	                    Map<String, String> mShipGroupTempIndexShipMethods = new HashMap();
	                    Map<String, String> mShipMethodKeys = new HashMap<>();
