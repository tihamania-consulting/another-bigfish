...
public class TaxServices {
    ...
    public static Map taxCalculations(DispatchContext dctx, Map context) 
    {
        ...
        if (UtilValidate.isNotEmpty(productStoreId)) 
        {
            try 
            {
                ...
                List taxAuthCondOrList = new ArrayList();
                ...
                else
                {
                    ...
                    while (flIt.hasNext()) 
                    {
                        ...
                        GenericValue taxAuthorityGlAccount = delegator.findOne("TaxAuthorityGlAccount", UtilMisc.toMap("taxAuthPartyId", taxAuthPartyId, "taxAuthGeoId", taxAuthGeoId, "organizationPartyId", payToPartyId));
    ...
    public static Set<String> expandGeoRegionDeep(Set<String> geoIdSet, Delegator delegator) throws GenericEntityException {
        ...
        for (String curGeoId: geoIdSet) {
            List<GenericValue> geoAssocList = delegator.findByAnd("GeoAssoc", UtilMisc.toMap("geoIdTo", curGeoId, "geoAssocTypeId", "REGIONS"));
