...
public class EmailServices {
    ...
    public static final VisualTheme visualTheme = ThemeFactory.getVisualThemeFromId("COMMON");


    protected static final FoFormRenderer foScreenRenderer = new FoFormRenderer();
    ...
    public static Map abandonCartEmail(DispatchContext dctx, Map context) {
        ...
        try {
            ...
            try {
                productStoreEmail = delegator.findOne("ProductStoreEmailSetting", UtilMisc.toMap("productStoreId", productStoreId, "emailType", emailType), true);
...
//            List<GenericValue> shoppingLists = delegator.findByAnd("ShoppingList", UtilMisc.toMap("productStoreId", productStoreId, "shoppingListTypeId", "SLT_SPEC_PURP"));
            ...
            for (GenericValue shoppingList : shoppingLists) 
            {
                ...
                {
                    GenericValue party = shoppingList.getRelatedOne("Party", true);
                    if (UtilValidate.isNotEmpty(party)) 
                    {
                        ...
                        if (gcStart.before(gcEnd)) 
                        {
                            ...
                            List<GenericValue> communicationEvents = delegator.findByAnd("CommunicationEvent", UtilMisc.toMap("partyIdTo", partyId, "reasonEnumId", "ABCART_EMAIL"), null, true);
                            ...
                            if (communicationEventsCount < emailCount) 
                            {
                                ...
                                if (UtilValidate.isNotEmpty(emailList)) 
                                {
                                    ...
                                    GenericValue person = party.getRelatedOne("Person", true);
                                    ...
                                    Debug.logInfo(sendTo, module);
                                    Map communicationEventMap = new HashMap();
...
//            sendResp = dispatcher.runSync("sendMailFromScreen", sendMap);
     ...
     */
    public static Map<String, Object> sendMailFromScreen(DispatchContext dctx, Map<String, ? extends Object> rServiceContext) throws IOException, TemplateException {
        ...
        if (UtilValidate.isEmpty(productStoreEmail) && UtilValidate.isNotEmpty(bodyScreenUri))
        {
            try
            {
            	List<GenericValue> productStoreEmailList = delegator.findByAnd("ProductStoreEmailSetting", UtilMisc.toMap("bodyScreenLocation", bodyScreenUri), null, false);
        ...
        ScreenRenderer screens = new ScreenRenderer(bodyWriter, screenContext,  new MacroScreenRenderer(modelTheme.getType("screen"), modelTheme.getScreenRendererLocation("screen")) );
        ...
        if (bodyScreenUri != null)
        {
            ...
            catch (GeneralException | ParserConfigurationException | SAXException | IOException e)
            {
        ...
        if (UtilValidate.isNotEmpty(attachmentFileLocation) && UtilValidate.isNotEmpty(attachmentFileName) && UtilValidate.isNotEmpty(attachmentFileMimeType))
        {
            try
            {
	            ...
	            List<Map<String, ? extends Object>> bodyParts = new ArrayList();
	            if (bodyText != null)
	            {
        ...
        else if (UtilValidate.isNotEmpty(xslfoAttachScreenLocation))
        {
            ...
            try
            {
                ...
                ScreenRenderer screensAtt = new ScreenRenderer(writer, screenContext, new MacroScreenRenderer(visualTheme.getModelTheme(), "screenfop"));
        ...
        try {
	            ...
	            else
	            {
		            if (UtilValidate.isEmpty(communicationEventId))
		            {
	            	    ...
	            	    createCommEventFromEmailCtx.put("userLogin", delegator.findOne("UserLogin", UtilMisc.toMap("userLoginId", "system"), false));
