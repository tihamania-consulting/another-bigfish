...
public class RefinementsHelperSolr {
    ...
    public Map<String, Integer> getResultFacetQueryPrice(HttpServletRequest request, List<SolrIndexDocument> results, List<SolrIndexDocument> multiSelectResults) 
    {
        ...
        List<Map<String, Integer>> resultsFacetQueriesMapList = new ArrayList();
        Map<String, Integer> resultsFacetQueriesMap = FastMap.newInstance();
        ...
        if (UtilValidate.isNotEmpty(priceLow) && UtilValidate.isNotEmpty(priceHigh)) 
        {
            ...
            while (priceRange > 0) 
            {
                ...
                Map<String, Integer> productCountsInRange = getProductCountsInRange(resultsDoc, rangeLowVal, rangeHighVal);
                int productCount = productCountsInRange.get(priceRangeMap);
                ...
                if (productCount > 0 || facetMultiSelectTrue)
                {
                    ...
                    while (tempPriceRange > 0 && !facetMultiSelectTrue) 
                    {
                        ...
                        Map<String, Integer> tempProductCountsInRange = getProductCountsInRange(resultsDoc, tempRangeLowVal, tempRangeHighVal);
                        int productCounttemp = tempProductCountsInRange.get(priceRangeQueryTemp);
    ...
    public Map getLowHighPriceMap(HttpServletRequest request, List<SolrIndexDocument> resultsDoc)
    {
        ...
        Map<String, Object> lowHighPriceMap = FastMap.newInstance();
    ...
    private GenericRefinementValue convertRefinementValueToGeneric(String key, long count, GenericRefinement genericRefinement) 
    {
        ...
        if (!isOnCategoryList) 
        {
            ...
            List<String> filterGroupList = new ArrayList();
    ...
    public List<SolrIndexDocument> getProductDocInPriceRange(List<SolrIndexDocument> results, Float lowPrice, Float highPrice)
    {
    	...
    	Iterator itr = results.iterator();
        Map<String, Integer> productCountMap = FastMap.newInstance();
        List newresult = new ArrayList();
        int count = 0;
        try 
        {
    ...
    public static List<SolrIndexDocument> updateSolrDocumentPrice(List<SolrIndexDocument> results, HttpServletRequest request) 
    {
		...
		List<GenericValue> productPriceRules = new ArrayList();
		if(UtilValidate.isNotEmpty(partyId))
		{
			List<GenericValue> partyIdConds = new ArrayList();
			try 
			{
				partyIdConds = delegator.findByAnd("ProductPriceCond", UtilMisc.toMap("inputParamEnumId", "PRIP_PARTY_ID", "condValue", partyId));
			} 
